...
def binarize(word_img):
    ...
    return binary_img // 255  # Ensuring output is binary (0s and 1s)
        ...
        for row in range(h - 1):
            for col in range(1, w - 1):
                if binary_img[row][col] == 0 and binary_img[row][col - 1] == 1 and binary_img[row][col + 1] == 1 and binary_img[row + 1][col] == 1 and VP[col] != 0:
                    ...
                    flag = 1  # Set flag to 1 to indicate a change was made
...
def horizontal_transitions(word_img, baseline_idx):
    ...
    return lines[len(lines) // 2] if lines else baseline_idx  # Safeguard against empty lines
    ...
    while i >= 0:
        ...
        if f == 1:
            if pixel == 0 and flag == 1:
                ...
                # Loop to find nearest VP = 0 and VP = MFV
                ...
                # Determine cut index based on found positions
                cut_index = determine_cut_index(VP, mid, left_zero, right_zero, left_MFV, right_MFV)
                ...
                top = np.argmax(HP != 0)  # Get top line of the segment

                cnt = sum(vertical_transitions(word_img, k) > 2 for k in range(end, cut_index + 1))
...
def determine_cut_index(VP, mid, left_zero, right_zero, left_MFV, right_MFV):
    if VP[mid] == 0:
        return mid
    elif left_zero != -1 and right_zero != -1:
        return left_zero if abs(left_zero - mid) <= abs(right_zero - mid) else right_zero
    elif left_zero != -1:
        return left_zero
    elif right_zero != -1:
        return right_zero
...
def inside_hole(word_img, end_idx, start_idx):
    ...
    for j in range(end_idx + 1, start_idx):
        if vertical_transitions(sk, j) <= 2:
            return 0
    return 1
...
def check_hole(segment):
    ...
    return sum(1 for hier in hierarchy[0] if hier[3] >= 0) != 0
...
def remove_dots(word_img, threshold=11):
    ...
    char = [label for label in range(1, components) if stats[label, cv.CC_STAT_AREA] > threshold]
...
def check_dots(segment):
    ...
    return len(contours) > 1  # Check if more than one contour is present
...
def safe_connected_components(binary_img):
    # Ensure input is a binary image (0s and 1s)
    if binary_img.max() > 1 or binary_img.min() < 0:
        print("Warning: Input image is not binary. Converting to binary...")
        binary_img = (binary_img > 0).astype(np.uint8)  # Convert to binary (0s and 1s)
    
    try:
        cc, labels = cv.connectedComponents(binary_img, connectivity=4)
        return cc, labels
...
def filter_regions(word_img, no_dots_copy, SRL:list, VP:list, upper_base:int, lower_base:int, MTI:int, MFV:int, top_line:int):
    ...
    components, labels= safe_connected_components(word_img)
